...
export default function PendingAdminResponseChats() {
  ...
  useEffect(() => {
    ...
    if (!isAuthenticated) {
      router.replace(`/login?next=${encodeURIComponent("/admin/chats/pending-admin-response")}`);
    }
  ...
  useEffect(() => {
    async function loadOpenChats() {
      ...
      try {
        setLoading(true);
        const data = await fetchJson<any[]>(`/api/Chats/open/pending-admin-response`);
        setChats(data);
      } catch (err: any) {
        console.error(err);
  ...
  return (
    <div className="max-w-5xl mx-auto py-10 px-4">
      <Card className="p-6">
        <div className="mt-6">
          <h2 className="text-lg font-semibold mb-3">Chats Pendentes de Resposta (Admin)</h2>
          {loading && (
