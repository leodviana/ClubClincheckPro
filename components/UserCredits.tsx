...
function findPatientName(obj: any, depth = 2): string | null {
  ...
  // shallow scan values for candidate strings
  for (const k of keys) {
    try {
      const v = obj[k];
      if (typeof v === "string" && String(v).trim() !== "" && re.test(k)) return String(v).trim();
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
          ...
          // debug: inspect raw response from endpoint
          try {
            // logs appear in the browser console (this is a client component)
            // eslint-disable-next-line no-console
            console.debug("[UserCredits] fetch result:", data);
          } catch (e) {}
          ...
          try {
            // eslint-disable-next-line no-console
            console.debug("[UserCredits] rawList:", rawList);
          } catch (e) {}
          ...
          const list: Credit[] = (rawList || []).map((it: any) => {
            ...
            try {
              // eslint-disable-next-line no-console
              console.debug("[UserCredits] mapped item (before object):", { id: idCandidate, foundPatient, titleCandidate, title });
            } catch (e) {}
            ...
            const mappedObj = {
              ...
              // Prefer timestamps from the case/caseInfo payload when available
            ...
            // Normalize empty strings to null so `formatDate` and UI show '-'
            ...
            try {
              // eslint-disable-next-line no-console
              console.debug("[UserCredits] mapped item (final):", mappedObj);
            } catch (e) {}
